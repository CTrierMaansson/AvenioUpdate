...
#' @importFrom dplyr select `%>%` filter group_by left_join count ungroup arrange mutate bind_rows
...
included_analyses <- function(master_list){
    ...
    combined_df <- combined_df %>% 
        dplyr::mutate(Analysis.Type = ifelse(grepl("tissue",sample_index),
                                             "Tissue",
                                             "Plasma"))
    ...
    combined_sele <- combined_df %>% 
        dplyr::select(Analysis.ID, Analysis.Name,Analysis.Type) %>% 
        unique()
