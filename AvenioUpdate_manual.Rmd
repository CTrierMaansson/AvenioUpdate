...
2. `Directory` which is the **complete** path (starts with "//Synology_m1") to the specific run on the Synology server
...
(names `list` of `data.frames`). This is automatically
...
As a default synonymous mutations are included in the output 
(`synonymous = TRUE`).
If the out should contain all the information from the AVENIO `.csv` files then
`simple = FALSE` (default). If you want a more simple output similar to the 
output from `create_simple_output()` then set `simple = FALSE`
