...
maintain a consistency across all our project. Yes! Very neat!
...
7. Avoid the use of special characters like "ÆØÅ½#$" etc. but "_" is okay
8. And do not delete anything you are not supposed to

### Formatting entries in AVENIO_runs.xlsx

This is an example of some the entries in the file:
...
**date correctly**. The date should be formatted as `YYYY-MM-DD`. 
It is quite often the sample date for buffy coat (BC) and baseline (BL) samples
...
specific Sample_name and if it does not it will result in an error.
...
be included in this column **however it is strongly advised to put in:**
**"BL","Tx","Unknown", or "BC"**
...
### More technical note on AVENIO_results_patients.rds
...
2. `Directory` which is the path to the specific run on the Synology server
...
The output of the function is the updated `master_list`. This is automatically
saved as the AVENIO_results_patients.rds file.
...
4. The fourth message explains which samples have been added to the dataset
where the samples are formatted as `Project_SampleName_Date_Material`
...
This is the second **main** function of the package and is used to get a quick 
...
If synonymous mutations should be excluded you can set `synonymous = FALSE`.
As a default synonymous mutations are included in the output 
(`synonymous = TRUE`).
...
1. **sample_index** - Unique for each sample and reflects Project, Name_in_project, Sample_date, and Material in the AVENIO_runs.xlsx file
...
overview <- create_simple_output(master_list = results,
                                 CPR = "1401362941",
                                 synonymous = F)
...
useful in order to get an overview of the samples, runs, etc. 

## `included_analyses()`
This function is used to get an overview of the runs from AVENIO_runs.xlsx that
are included in the results dataset. 
The idea with this function is to use it to check if all the samples from have 
in fact been included in the dataset.
...
The function takes **NO** mandatory arguments, but takes two optional arguments:
...
extracts information from the Avenio_results_patients.rds file generates
...
samples that have not been added to the dataset but are present in the 
